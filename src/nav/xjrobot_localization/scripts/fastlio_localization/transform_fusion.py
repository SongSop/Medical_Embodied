...
class TransformFusion(Node):
    def __init__(self):
        ...
        self.create_subscription(Odometry, self.get_parameter("odom_topic").value, self.cb_save_cur_odom, 1)
    ...
    def transform_fusion(self):
        ...
        transform_stamped_msg = tf2_ros.TransformStamped(
                ...
                child_frame_id = self.get_parameter("map_child_frame").value,
                transform = transform_msg
            )
        transform_stamped_msg.header.frame_id = self.get_parameter("map_frame").value
        ...
        if cur_odom is not None:
            ...
            localization.child_frame_id = cur_odom.child_frame_id
