...
## RoboSense 本地部署（按当前需求）

以下默认值已写入新 launch：`hdl_localization_rslidar_local.launch.py`，直接匹配：

- 里程计话题：`/odom/unfiltered`
- 里程计坐标：`frame_id=odom`，`child_frame_id=base_footprint`
- 点云话题：`/rslidar_points`
- 旋转后 IMU 话题：`/imu/link1`
- 地图文件：`/home/medical/rs_lidar/src/FAST_LIO/PCD/test.pcd`

本仓库内 `rslidar_sdk` 默认配置显示点云 `frame_id` 为 `rslidar`（`ros_frame_id: rslidar`），因此本地部署采用：

- `base_footprint -> rslidar`（静态）
- `base_footprint -> link1`（静态）

坐标关系文件已提供在：

```text
src/hdl_localization/config/rslidar_local_frames.yaml
```

### 启动命令（RoboSense）

```bash
cd /home/medical/rs_lidar
source /opt/ros/jazzy/setup.bash
source install/setup.bash

ros2 launch hdl_localization hdl_localization_rslidar_local.launch.py
```
...
ros2 topic echo /imu/link1 --once
ros2 topic echo /odom/unfiltered --once
```

重点检查：

- `/rslidar_points` 的 `header.frame_id` 是否为 `rslidar`
- `/imu/link1` 的 `header.frame_id` 是否与 `link1` 对齐（若你的 IMU 旋转节点另有设置，以实测为准）
- `/odom/unfiltered` 的 `header.frame_id=odom` 与 `child_frame_id=base_footprint`
