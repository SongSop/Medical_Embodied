...
public:
  ...
  : Node("hdl_localization", options)
  {
    ...
    const double log_output_frequency_hz = declare_parameter<double>("log_output_frequency_hz", 1.0);
    log_throttle_ms = log_output_frequency_hz > 0.0 ?
      std::max(1, static_cast<int>(1000.0 / log_output_frequency_hz)) :
...
private:
  ...
  void points_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr points_msg) {
    if (!globalmap) {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), log_throttle_ms, "globalmap has not been received!!");
      return;
    }
    ...
    if (pcl_cloud->empty()) {
      RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), log_throttle_ms, "cloud is empty!!");
      return;
    }
...

    {
      ...
      if (!pose_estimator) {
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), log_throttle_ms, "waiting for initial pose input!!");
        return;
      }
